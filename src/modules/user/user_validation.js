...
const signupSchema = Joi.object({
  ...
  mobile: Joi.string()
    .pattern(new RegExp("^01[0-2][0-9]{8}$"))
    .required()
    .messages({
...
const changePasswordSchema = Joi.object({
  newPassword: Joi.string()
    ...
    .messages({
      "string.pattern.base":
        "new Password must contain at least two uppercase letters and one special character",
...
const settingsProfile = Joi.object({
  birthOfDate: Joi.string()
    ...
    .messages({
      "string.pattern.base":
        "Date of birth must be in the format like 15/08/1990",
    }),
  ...
  profileImage: Joi.object({
    ...
    size: Joi.number()
      .max(5 * 1024 * 1024)
      .required(), // Limit file size to 5MB
  }),
});
